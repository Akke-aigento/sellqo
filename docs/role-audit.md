...
- Autorisatie in de function: header `Authorization: Bearer <SUPABASE_SERVICE_ROLE_KEY>` verplicht, anders 401. `verify_jwt=false` in `supabase/config.toml` omdat de function via `net.http_post` vanuit SQL wordt aangeroepen, exact zoals de bestaande cron-endpoints — de sleutel-check in de function is de sluitpost, niet de JWT-laag.
