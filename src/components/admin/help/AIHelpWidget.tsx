...
export function AIHelpWidget() {
  ...
  return (
    <>
      ...
      {!isOpen && !isMinimized && (
        <div className="fixed bottom-[var(--admin-nav-offset)] md:bottom-4 right-4 z-50 flex items-center gap-2">
          <Button
            onClick={() => {
              setIsMinimized(true);
              localStorage.setItem('ai-help-minimized', 'true');
      ...
      {!isOpen && isMinimized && (
        <Button
          ...
          size="icon"
          variant="outline"
          className="fixed bottom-[var(--admin-nav-offset)] md:bottom-4 right-4 z-50 h-9 w-9 rounded-full shadow-md bg-background/80 backdrop-blur-sm"
          title={t('admin.help.aIHelpChatWindow.sellqo_assistent')}
        >
