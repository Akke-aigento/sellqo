      ...
      <div className="flex justify-center">
        ...
        >
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  {logoUrl ? (
                    ...
                    <p className="text-[9px] font-bold" style={{ fontFamily: `"${headingFont}", serif` }}>
                      {shopName}
                    </p>
                  )}
                </div>
          ...
          >
            ...
            <div
              className="inline-block rounded-full px-3 py-0.5 text-[7px] font-medium text-white"
          ...
          <div className="p-3">
            ...
            <div
              ...
              style={{ gridTemplateColumns: `repeat(${Math.min(device === 'mobile' ? 2 : productsPerRow, 4)}, 1fr)` }}
            >
              {visibleProducts.map((product, i) => (
                <div key={i} className={cn(
                  'group',
                  productCardStyle === 'detailed' && 'space-y-1.5',
                  productCardStyle === 'minimal' && 'space-y-0.5',
                  productCardStyle === 'standard' && 'space-y-1',
                )}>
                  <div
                    className="aspect-square rounded-md relative overflow-hidden"
                    style={{ backgroundColor: secondaryColor + '20' }}
                  >
                    {/* Mock product image pattern */}
                    <div className="absolute inset-0 flex items-center justify-center opacity-20">
                      <div className="w-1/2 h-1/2 rounded-full" style={{ backgroundColor: primaryColor }} />
                    </div>
                    {product.oldPrice && (
                      <div
                        className="absolute top-1 left-1 px-1 rounded text-[5px] font-bold text-white"
                  ...
                  {productCardStyle !== 'minimal' && (
                    <div className="flex items-center gap-1">
                      <span className="text-[7px] font-bold" style={{ color: accentColor }}>
                        {product.price}
                      </span>
                      {product.oldPrice && (
                        <span className="text-[6px] line-through opacity-40">{product.oldPrice}</span>
                      )}
                    </div>
                  )}
                  {productCardStyle === 'detailed' && (
                    <p className="text-[5px] opacity-50 line-clamp-2">
                      Premium kwaliteit met oog voor detail en duurzame materialen.
                    </p>
                  )}
                </div>
              ))}
