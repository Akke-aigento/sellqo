...
import {
  ...
  ExternalLink, Download, Code, Send,
} from 'lucide-react';
...
export function CustomFrontendConfigPanel({ config, onChange }: CustomFrontendConfigPanelProps) {
  ...
  // Simulated API key (will be real when API key generation is implemented)
  const apiKeyPrefix = 'sk_live_';
  const maskedKey = `${apiKeyPrefix}${'•'.repeat(24)}`;
  const displayKey = showApiKey ? `${apiKeyPrefix}xxxxxxxxxxxxxxxxxxxxxxxx` : maskedKey;
  ...
  const generateCodeSnippet = (framework: string): string => {
    const enabledModules = FEATURE_MODULES.filter(m => isModuleEnabled(m.id));
    ...
    const baseCode = `// sellqo-client.ts — Auto-generated by SellQo
const SELLQO_API_BASE = '${apiBaseUrl}';
const SELLQO_TENANT_ID = '${tenantSlug}';
const SELLQO_API_KEY = '${showApiKey ? 'sk_live_xxxxxxxx' : 'sk_live_••••••••'}';
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex gap-2">
              <div className="flex-1 relative">
                <Input value={displayKey} readOnly className="font-mono text-sm pr-10" />
                <button
                  type="button"
              ...
              <Button variant="outline" size="icon" onClick={() => copyToClipboard(displayKey, 'api-key')}>
          ...
          <div className="flex items-center gap-2 p-3 rounded-lg bg-muted/50">
            <div className={cn("h-2.5 w-2.5 rounded-full", config.frontend_url ? 'bg-green-500' : 'bg-amber-400')} />
            <span className="text-sm font-medium">
              {config.frontend_url ? 'Wacht op eerste API-verzoek' : 'Wacht op configuratie'}
            </span>
          </div>
        </CardContent>
      </Card>
