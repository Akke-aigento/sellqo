...
import { Check, X, Star, ArrowUp, ArrowDown, Minus } from 'lucide-react';
...
function compareFeatures(
  ...
  for (const key of Object.keys(currentFeatures) as (keyof PricingPlanFeatures)[]) {
    ...
    if (!currentHas && targetHas) {
      gained.push(featureLabels[key] || key);
    } else if (currentHas && !targetHas) {
      lost.push(featureLabels[key] || key);
    }
  }
        ...
        {sortedPlans.map((plan, index) => {
          ...
          if (currentPlan) {
            if (plan.limit_products !== currentPlan.limit_products) {
              limitChanges.push({
                label: 'Producten',
                current: currentPlan.limit_products,
                target: plan.limit_products,
                isIncrease: (plan.limit_products || Infinity) > (currentPlan.limit_products || Infinity),
              });
            }
            if (plan.limit_orders !== currentPlan.limit_orders) {
              limitChanges.push({
                label: 'Orders/mnd',
                current: currentPlan.limit_orders,
                target: plan.limit_orders,
                isIncrease: (plan.limit_orders || Infinity) > (currentPlan.limit_orders || Infinity),
              });
            }
            if (plan.limit_customers !== currentPlan.limit_customers) {
              limitChanges.push({
                label: 'Klanten',
                current: currentPlan.limit_customers,
                target: plan.limit_customers,
                isIncrease: (plan.limit_customers || Infinity) > (currentPlan.limit_customers || Infinity),
              });
            }
            if (plan.limit_users !== currentPlan.limit_users) {
              limitChanges.push({
                label: 'Teamleden',
                current: currentPlan.limit_users,
                target: plan.limit_users,
                isIncrease: plan.limit_users > currentPlan.limit_users,
              });
            }
          }
          ...
          return (
            <Card
              ...
              className={cn(
                ...
                plan.highlighted && !isCurrent && 'border-primary shadow-lg',
            ...
            >
              ...
              <CardContent className="flex-1 space-y-4">
                ...
                {/* Feature changes (only for non-current plans) */}
