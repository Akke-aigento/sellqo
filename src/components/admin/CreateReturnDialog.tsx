...
export function CreateReturnDialog({
  ...
  const [items, setItems] = useState<ItemDetail[]>(() =>
    orderItems.map((oi) => {
      ...
      const fee = (oi.price * defaultRestockingPercent) / 100;
      return {
        selected: false,
        ...
        restockingFee: fee,
        refundAmount: maxQty * oi.price - fee * maxQty,
        expanded: false,
  ...
  // Totals
  const subtotalItems = selectedItems.reduce((s, it, _i) => {
    const origIdx = items.indexOf(it);
    return s + it.quantity * orderItems[origIdx].price;
  }, 0);
  ...
  const handleSubmit = () => {
    ...
    items.forEach((it, idx) => {
      ...
      returnItems.push({
        ...
        unit_price: oi.price,
        line_total: it.quantity * oi.price,
                  ...
                  <div key={index} className="border rounded-md">
                    <div className="flex items-center gap-3 p-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">{formatCurrency(oi.price)} × max {maxQty}</p>
                      </div>
                      {item.selected && (
                        <>
                          <Input
                      ...
                      <div className="px-3 pb-3 pt-1 border-t space-y-3">
                          ...
                          <DefaultIndicator
                            ...
                            onReset={() => {
                              ...
                              const refund = item.refundAmountOverridden
                                ? item.refundAmount
                                : recalcRefundAmount(index, item.quantity, fee);
                              updateItem(index, { restockingFee: fee, restockingFeeOverridden: false, refundAmount: refund });
                            }}
                          />
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground mt-0.5">
                            Berekend: {formatCurrency(recalcRefundAmount(index, item.quantity, item.restockingFee))} ({item.quantity} × {formatCurrency(oi.price)} − {formatCurrency(item.restockingFee)} fee)
                          </p>
                          <DefaultIndicator
