      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList>
          <TabsTrigger value="crm">
            ...
            CRM Klanten
            <Badge variant="secondary" className="ml-1.5 text-xs">{filteredCustomers.length}</Badge>
          </TabsTrigger>
          <TabsTrigger value="storefront">
            <Globe className="h-4 w-4 mr-1.5" />
            Webshop Accounts
            <Badge variant="secondary" className="ml-1.5 text-xs">{storefrontCustomers.length}</Badge>
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="storefront">
          <Card>
            <CardHeader className="pb-3">
              ...
              <CardDescription>
                {storefrontCustomers.length} account{storefrontCustomers.length !== 1 ? 's' : ''} geregistreerd via de webshop
              </CardDescription>
            </CardHeader>
            <CardContent className="overflow-x-auto px-0 sm:px-6">
              {sfLoading ? (
              ...
              ) : storefrontCustomers.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  <Globe className="h-12 w-12 text-muted-foreground mb-4" />
                  <h3 className="font-medium text-lg">Geen webshop accounts</h3>
                  <p className="text-muted-foreground text-sm">
                    Klanten verschijnen hier wanneer ze een account aanmaken op de webshop
                  </p>
                </div>
              ) : (
                <div className="overflow-x-auto -mx-4 sm:mx-0"><div className="min-w-[550px]">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead>Klant</TableHead>
                      <TableHead className="hidden lg:table-cell">Contact</TableHead>
                      <TableHead className="hidden sm:table-cell">Nieuwsbrief</TableHead>
                      <TableHead className="hidden sm:table-cell">Status</TableHead>
                      <TableHead className="hidden md:table-cell">Laatste login</TableHead>
                      <TableHead className="hidden md:table-cell">Geregistreerd</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {storefrontCustomers.map((sc) => (
                      <StorefrontCustomerRow key={sc.id} customer={sc} />
                    ))}
                  </TableBody>
                </Table>
                </div></div>
...
function StorefrontCustomerRow({ customer }: { customer: StorefrontCustomer }) {
    ...
    <TableRow>
      ...
      <TableCell className="hidden md:table-cell text-muted-foreground text-sm">
        {customer.last_login_at 
          ? format(new Date(customer.last_login_at), 'd MMM yyyy', { locale: nl })
          : '-'}
      </TableCell>
      <TableCell className="hidden md:table-cell text-muted-foreground text-sm">
