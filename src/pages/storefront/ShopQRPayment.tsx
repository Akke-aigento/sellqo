...
export default function ShopQRPayment() {
  ...
  const { orderId, orderNumber, total, currency = "EUR", bankDetails, qrData } = state;
  const qrPayload = qrData?.payload;
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-muted/30 px-4 py-8">
      <div className="max-w-md w-full bg-background rounded-2xl shadow-lg p-8 text-center space-y-6">
          ...
          <>
            ...
            <div className="flex justify-center">
              <div className="bg-white border-2 border-border rounded-xl p-4">
                <QRCode value={qrPayload} size={224} level="M" />
              </div>
            </div>
          </>
        ) : (
          <div>
            <h1 className="text-2xl font-bold text-foreground">Betaal je bestelling</h1>
            <p className="text-muted-foreground mt-1">
              Maak het bedrag over met onderstaande gegevens
            </p>
          </div>
        )}
        ...
        {!qrPayload && (
          <div className="text-left text-sm bg-muted rounded-lg p-4 space-y-1">
        ...
        {/* Fallback: manual bank details (only shown when QR present) */}
        {qrPayload && (
          <Collapsible>
