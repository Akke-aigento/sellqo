  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from('orders')
        .select('id, order_number, customer_name, customer_email, total, status, payment_status, created_at, marketplace_source, marketplace_connection_id, shipping_cost, subtotal, discount_amount, discount_code')
