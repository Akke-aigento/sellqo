...
Elke nieuwe SellQo custom frontend (Lovable-project voor een specifieke tenant) MOET bij aanmaak de volgende patronen correct implementeren. Dit runbook voorkomt dat bekende bugs opnieuw moeten worden uitgevogeld.
...
Met proxy (Loveke/VanXcel): pad `/get_shipping_countries` met **underscore**. Direct (Astra): `call("get_shipping_countries", {})` — NIET via `callCart` (geen cart-response).
...
- Referentie-implementatie: SellQo-core `src/lib/shippingRegions.ts` (`localizedCountryOptions`).
...
## PATROON 5 — Mobiele layout-veiligheid (MOBILE-OVERFLOW-1)

**Root cause:** een grid- of flex-item krijgt standaard `min-width: auto`, dus het weigert kleiner te worden dan de min-content-breedte van zijn inhoud. Een horizontaal scrollende strip (bv. een thumbnail-galerij: `flex gap-3 overflow-x-auto` met 6× `w-20`) forceert daardoor zijn parent-kolom naar de volle inhoudsbreedte (bv. 540px), i.p.v. te scrollen. Dat rekt container → body → document op tot voorbij de viewport-breedte. Gevolg op mobiel: de browser **zoomt de hele pagina uit** (uitzoomen met twee vingers "lost het op"), en een `aspect-square`-hoofdbeeld dat de opgeblazen kolombreedte volgt lijkt sterk uitvergroot. Het uitvergrote beeld is het symptoom, de niet-krimpende kolom de oorzaak.

### 5a — Regels

- Elk grid-/flex-item dat een `overflow-x-auto`-strip (of andere brede inhoud) bevat, MOET `min-w-0` krijgen. Zonder `min-w-0` scrolt de strip nooit — hij blaast de kolom op.
- Zet `overflow-x-hidden` als vangnet op de pagina-`<main>` (alle returns: loading, niet-gevonden, hoofd). Eén lekkend element mag nooit de hele viewport laten inzoomen.
- Verifieer met een `viewport`-meta die correct is (`width=device-width, initial-scale=1`) — maar besef: een correcte meta beschermt NIET tegen een te breed kind-element. De `min-w-0` + `overflow-x-hidden` doen dat wel.
- Diagnose bij "hele pagina is ingezoomd op mobiel": meet met Playwright op 390px of `document.documentElement.scrollWidth > clientWidth`. Zo ja, bisect welk element de breedte oprekt (verberg kinderen één voor één en kijk wanneer `scrollWidth` daalt). Gok niet — meet.

### 5b — Let op

- Lovable-mockup rendert breder dan 640px, dus `md:`-breakpoints activeren daar wél maar op een echte telefoon (~390px) niet. Verifieer overflow altijd op echt mobiel formaat, niet in de mockup.
- Framer-Motion `x`-transforms (`initial={{ x: ±20 }}`) zijn meestal NIET de oorzaak van dit type overflow — controleer eerst de grid/flex-min-width vóór je animaties aanpast.
...
- `df49743` — verzendkeuze resetten bij checkout-start (SHIP-RESET-1, stale gratis-methode)
