...
async function checkoutStart(supabase: any, tenantId: string, params: Record<string, unknown>) {
  ...
  if (resetError) {
    console.error('[checkoutStart] Reset update failed:', resetError);
  }
...
async function checkoutVerifyPayment(supabase: any, tenantId: string, params: Record<string, unknown>) {
  ...
  // NOTIF-SOURCES-1: geen eigen melding meer. De trigger handle_order_notification
  // maakt order_new zodra de order betaald is; hier stond ~9 s later een tweede
  // (storefront_order_new, niet instelbaar). Request en response ongewijzigd.
