...
async function fetchAllOffers(accessToken: string): Promise<BolOfferItem[]> {
  ...
  while (hasMore) {
    ...
    // The export endpoint is async - fall back to paginated offers listing
    // Use the offers endpoint with pagination instead
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { connectionId, mode = 'list', selectedOfferIds } = body
    ...
    // Fetch all offers from Bol.com
    const bolOffers = await fetchAllOffers(accessToken)

    if (mode === 'list') {
      // Get existing products to check EAN matches
      const { data: existingProducts } = await supabase
        .from('products')
        .select('id, name, bol_ean, bol_offer_id, stock')
        .eq('tenant_id', connection.tenant_id)

      const existingEanMap = new Map<string, { id: string; name: string; bol_offer_id: string | null }>()
      for (const p of existingProducts || []) {
        if (p.bol_ean) {
          existingEanMap.set(p.bol_ean, { id: p.id, name: p.name, bol_offer_id: p.bol_offer_id })
        }
      }

      // Map offers for frontend
    ...
    // Get existing products for EAN dedup
    const { data: existingProducts } = await supabase
      .from('products')
    ...
    for (const offer of offersToImport) {
      try {
        ...
        if (existingId) {
          ...
          linked++
          console.log(`Linked existing product ${existingId} to offer ${offer.offerId}`)
        } else {
          // Create new product
          ...
          if (insertError) {
            console.error(`Error importing offer ${offer.offerId}:`, insertError)
            errors++
            continue
          }

          imported++
          console.log(`Imported new product for offer ${offer.offerId}`)
        }
      } catch (err) {
        console.error(`Error processing offer ${offer.offerId}:`, err)
        errors++
      }
    }
    ...
    return new Response(
      JSON.stringify({
        ...
        linked,
        errors,
        totalProcessed: imported + linked,
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
