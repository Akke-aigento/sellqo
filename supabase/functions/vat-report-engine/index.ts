...
/*
    if (!body.force_recompute) {
      ...
      await supabase
        ...
        .gt('computed_at', new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString())
        .maybeSingle();
      console.log(`[perf] cache_select=${Date.now() - tCache}ms`);
      if (cached?.payload) {
        const tHit = Date.now();
        ...
        payload.metadata.duration_ms = Date.now() - startedAt;
        if (!Array.isArray(payload.audit_trail)) payload.audit_trail = [];
        console.log(`[perf] cache_hit_total=${Date.now() - startedAt}ms (post_select=${Date.now() - tHit}ms)`);
        return new Response(JSON.stringify({ success: true, payload }), {
          headers: { ...cors, 'Content-Type': 'application/json', 'X-Cache': 'HIT' },
        });
      }
    ...
    const { data: tenant, error: tenantErr } = await supabase
      ...
      .eq('id', body.tenant_id)
      .maybeSingle();
    console.log(`[perf] tenant_select=${Date.now() - tTenant}ms`);
    if (tenantErr || !tenant) {
      return new Response(JSON.stringify({ success: false, error: 'Tenant not found' }), {
        status: 404, headers: { ...cors, 'Content-Type': 'application/json' },
      });
    ...
    const { data: invoiceRows, error: invErr } = await supabase
      ...
      .lte('issue_date', body.period_end)
      .in('status', statuses);
    console.log(`[perf] invoices_select=${Date.now() - tInv}ms count=${invoiceRows?.length ?? 0}`);
    if (invErr) throw invErr;
    const invoices = (invoiceRows || []) as unknown as DbInvoice[];
    ...
    console.log(`[perf] invoice_lines_select=${Date.now() - tLines}ms batches=${lineBatchCount} rows=${lineRowCount}`);
    ...
    console.log(`[perf] credit_notes_select=${Date.now() - tCn}ms count=${cnRows?.length ?? 0}`);
    if (cnErr) throw cnErr;
    const creditNotes = (cnRows || []) as unknown as DbCreditNote[];

    // 6) Credit note lines (batched)
    const tCnLines = Date.now();
    const cnLinesByNote = new Map<string, DbCreditNoteLine[]>();
    const cnIds = creditNotes.map((c) => c.id);
    ...
    console.log(`[perf] cn_lines_select=${Date.now() - tCnLines}ms`);
    ...
    if (typeof EdgeRuntime !== 'undefined' && EdgeRuntime?.waitUntil) {
      ...
      // Fallback: detach so we don't block response
      cachePromise.catch(() => {});
    }
