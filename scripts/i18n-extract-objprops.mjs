...
/** Offsets die in een comment of template literal vallen. */
function buildMask(src) {
  ...
  for (let i = 0; i < src.length; i++) {
    ...
    if (mode === null) {
      if (ch === '/' && next === '/') { mode = 'line'; start = i; }
      else if (ch === '/' && next === '*') { mode = 'block'; start = i; }
      else if (ch === '`') { mode = 'tpl'; start = i; }
    } else if ((mode === 'line' && ch === '\n')
            || (mode === 'block' && ch === '/' && src[i - 1] === '*' && i > start + 1)
            || (mode === 'tpl' && ch === '`' && src[i - 1] !== '\\' && i > start)) {
      mask.fill(1, start, i + 1);
      mode = null;
    }
  }
  if (mode !== null) mask.fill(1, start);
  return mask;
}
